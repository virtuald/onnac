...
// output a tar file
//	$content_array = array($file_contents,$file_name,$file_time)
//
function output_tar_file($content_array,$fName){
	
	global $cfg;
	
	// create a temporary file
	$tmpFile = tempnam('/tmp',"oe");

	$tar_object = new Archive_Tar($tmpFile,"gz");
	if (!$tar_object->create(''))
		return onnac_error("Error creating archive!");

	// add files to tar
	foreach ($content_array as $content)
		$tar_object->addstring($content[0],$content[1],$content[2]);
...
class Archive_Tar
{
    ...
    */
    function Archive_Tar($p_tarname, $p_compress = null)
    {
		...
		// register the destructor, since PEAR would normally do that
		register_shutdown_function(array(&$this,'_Archive_Tar'));
    }
    // }}}
    ...
    * @access public
    */
    function addString($p_filename, $p_string, $p_time = '')
    {
        $v_result = true;
        ...
        $v_result = $this->_addString($p_filename, $p_string, $p_time);
    ...
    */
    ...
    // {{{ _addString()
    function _addString($p_filename, $p_string, $p_time = '')
    {
	
	  if ($p_time == '' || !is_int($p_time))
	      $p_time = time();
